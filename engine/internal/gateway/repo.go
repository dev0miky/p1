...
const cols = `id, name, description, proxy, register, username,
  ...
  transport, media_encryption, dial_prefix, expire_seconds, retry_seconds, caller_id_in_from, extra_params,
...
func scan(row pgx.Row, g *Gateway, extra ...any) error {
	...
	dest := []any{
		...
		&g.MediaEncryption, &g.DialPrefix, &g.ExpireSeconds, &g.RetrySeconds, &g.CallerIDInFrom, &raw,
...
func (r *Repo) CreateTx(ctx context.Context, tx pgx.Tx, g Gateway) (Gateway, error) {
	...
	row := tx.QueryRow(ctx, `
		INSERT INTO gateways (name, description, proxy, register, username,
		  password_enc, realm, from_user, from_domain, transport, media_encryption, dial_prefix,
		...
		VALUES ($1,$2,$3,$4,$5,
		  ...
		  $7,$8,$9,$10,$16,$18,$11,$12,$13,$14,$17)
		RETURNING `+cols,
		g.Name, g.Description, g.Proxy, g.Register, g.Username,
		g.Password, g.Realm, g.FromUser, g.FromDomain, g.Transport,
		g.ExpireSeconds, g.RetrySeconds, g.CallerIDInFrom, extra, r.encKey, g.MediaEncryption, g.Enabled, g.DialPrefix,
	)
...
func (r *Repo) UpdateTx(ctx context.Context, tx pgx.Tx, id int64, p UpdatePatch) (Gateway, error) {
	...
	row := tx.QueryRow(ctx, `
		UPDATE gateways SET
		  ...
		  dial_prefix       = CASE WHEN $18::text IS NULL THEN dial_prefix ELSE $18 END,
		  updated_at        = now()
		WHERE id=$16
		RETURNING `+cols,
		...
		p.CallerIDInFrom, extra, r.encKey, p.Enabled, id, p.MediaEncryption, p.DialPrefix,
	)
	err := scan(row, &out)
	if errors.Is(err, pgx.ErrNoRows) {
		return out, ErrNotFound
...
func (r *Repo) ActiveGatewayTx(ctx context.Context, tx pgx.Tx) (name string, prefix string, err error) {
	err = tx.QueryRow(ctx, `SELECT name, dial_prefix FROM gateways WHERE is_active AND enabled LIMIT 1`).Scan(&name, &prefix)
	if errors.Is(err, pgx.ErrNoRows) {
		return "", "", nil
	}
